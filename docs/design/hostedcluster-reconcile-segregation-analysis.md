...
- Pull secret sync failure -> reported, but secret encryption, audit webhook, etcd MTLS,
  and global config sync all proceed independently (Phase 6 independence)
- Platform credential error -> reported, but kubeconfig sync, RBAC, operator deployments,
  and auxiliary operations all continue (Phase 6 vs Phase 8 independence)
- Kubeconfig sync failure -> reported, but kubeadmin password sync still runs (within
  reconcileKubeconfigAndPasswordSync wrapper)
- PKI RBAC failure -> reported, but prometheus RBAC and network policies still reconcile
  (within reconcileRBACAndPolicies wrapper)
- CPO deployment failure -> reported, but CAPI manager, CAPI provider, and karpenter
  operator all still deploy (within reconcileOperatorDeployments wrapper)
